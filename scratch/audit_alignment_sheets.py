...
def parse_sheet_rows(filepath, sheet_name):
...
# Let's inspect the sheets and their project metadata in all files
files = [
    'AOP alignment.xlsx',
    'AOP alignment2.xlsx',
    'AOP alignment Health and WASH projects.xlsx'
]
...
for f in files:
    ...
    with zipfile.ZipFile(fp) as z:
        wb_root = ET.fromstring(z.read('xl/workbook.xml'))
        main_ns = {'ns': 'http://schemas.openxmlformats.org/spreadsheetml/2006/main'}
        sheets = [s.attrib['name'] for s in wb_root.findall('.//ns:sheet', main_ns)]
        for s in sheets:
            project_sheets.append((f, s))

print(f"Total project sheets found: {len(project_sheets)}")
for f, s in project_sheets:
    rows = parse_sheet_rows(os.path.join(base_dir, f), s)
    # look for project title / donor
    meta = {}
    for r_idx, r in rows[:8]:
        line = " ".join([str(x) for x in r if x])
        if 'project' in line.lower():
            for c in r:
                if isinstance(c, str) and ('donor' in c.lower() or 'name' in c.lower() or 'title' in c.lower() or 'budget' in c.lower()):
                    pass
    print(f"  [{f}] -> Sheet: '{s}' (Rows: {len(rows)})")
