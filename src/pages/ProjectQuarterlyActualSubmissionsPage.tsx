// src/pages/ProjectQuarterlyActualSubmissionsPage.tsx
...
interface ProjectActualQuarterGroup {
  pe: PlanEntry;
...
export const ProjectQuarterlyActualSubmissionsPage: React.FC = () => {
  const {
    projects,
    nationalActivities,
    quarterlyActuals,
                          ...
                          return (
                            <tr key={qId} className="border-t hover:bg-slate-50">
                              ...
                              <td className="p-3 text-right whitespace-nowrap min-w-28 font-semibold">
                                {qa.actual.toLocaleString()} <span className="text-slate-400 font-normal">{pe.uom || na?.uom || ''}</span>
                              </td>
      ...
      {rejecting && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 flex items-center justify-center p-4">
          <div className="bg-white max-w-md w-full rounded-xl shadow-2xl p-5">
            <div className="text-sm font-black text-slate-800">Reject Project Quarterly Actual — {rejecting.quarter_id}</div>
            <textarea
              value={rejectReason}
