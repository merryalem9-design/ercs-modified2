...
import { PlanEntry, NationalActivity, Zone, QuarterlyActual, QuarterId } from '../types';
import { ShieldCheck, ChevronDown, ChevronRight } from 'lucide-react';

const ALL_QUARTER_IDS: QuarterId[] = ['Q1', 'Q2', 'Q3', 'Q4'];

interface ActualQuarterGroup {
  pe: PlanEntry;
...
export const QuarterlyActualSubmissionsPage: React.FC = () => {
  ...
  const groups: ActualQuarterGroup[] = entries
    .map(pe => {
    ...
    .filter((g): g is ActualQuarterGroup => g !== null)
  ...
  const submittedSlotCount = groups.reduce(
    (sum, g) => sum + g.slots.filter(s => s.qa !== undefined).length,
    0
  );
                          ...
                          if (!qa) {
                            return (
                              <tr key={qId} className="border-t border-slate-50 bg-white">
                                ...
                                <td className="p-3 text-slate-300 italic" colSpan={5}>Not submitted</td>
                              </tr>
                            );
                          }
                          ...
                          return (
                            <tr key={qId} className="border-t hover:bg-slate-50">
                              ...
                              <td className="p-3 text-right whitespace-nowrap min-w-28 font-semibold">
                                {qa.actual.toLocaleString()} <span className="text-slate-400 font-normal">{na?.uom}</span>
                              </td>
                              <td className="p-3 text-right whitespace-nowrap min-w-32 text-slate-600">
                                ETB {qa.expenditure.toLocaleString()}
                              </td>
                              <td className="p-3 min-w-40 text-slate-600 max-w-48 truncate">
                                {qa.comment || <span className="text-slate-300">—</span>}
                              </td>
                              <td className="p-3 text-center min-w-36">
                                ...
                                {qa.approval_status === 'Rejected' && qa.rejection_reason && (
                                  <div className="text-[9px] text-rose-600 mt-1 max-w-40 mx-auto">{qa.rejection_reason}</div>
                                )}
                              </td>
                              <td className="p-3 text-center min-w-44">
                                {qa.approval_status === 'Pending Approval' ? (
                                  <div className="flex items-center justify-center gap-2">
                                    <button
                                      onClick={e => { e.stopPropagation(); approveQuarterlyActual({ plan_entry_id: pe.id, quarter_id: qa.quarter_id }); }}
                                      className="px-2.5 py-1 rounded bg-emerald-50 text-emerald-700 font-bold hover:bg-emerald-100 text-[11px]"
                                    >
                                      Approve
                                    </button>
                                    <button
                                      onClick={e => { e.stopPropagation(); setRejecting({ plan_entry_id: pe.id, quarter_id: qa.quarter_id }); setRejectReason(''); }}
